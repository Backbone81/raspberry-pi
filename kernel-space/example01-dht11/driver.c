...
static int my_open(struct inode* inode, struct file* filep)
{
    ...
    {
        ERROR_OUT("Unable to allocate provate data");
        return -1;
    }
    ...
    snprintf(filep->private_data, BUFFER_SIZE, "Temperature: %i / Humidity: %i\n", my_data.temperature, my_data.humidity);
    printk(KERN_INFO "Temperature: %i / Humidity: %i\n", my_data.temperature, my_data.humidity);
...
static int my_init(void)
{
    ...
    error = alloc_chrdev_region(&device_number, 0, 1, "example01-dht11");
    if (error)
    {
    ...
    if (error)
    {
        ERROR_OUT("Unable to initialize GPIO");
